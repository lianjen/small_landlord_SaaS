...
try:
    ...
    def log_db_operation(op, table, success, count=0, error=None):
        if success:
...
def get_connection_pool():
    ...
    # 從 Streamlit secrets 讀取配置
    config = {
        'host': st.secrets.get('supabase', {}).get('host'),
        'port': st.secrets.get('supabase', {}).get('port', 5432),
        'database': st.secrets.get('supabase', {}).get('database'),
        'user': st.secrets.get('supabase', {}).get('user'),
        'password': st.secrets.get('supabase', {}).get('password'),
        'min_connections': 2,
        'max_connections': 10
    }
    
    if not pool_instance.is_initialized():
        pool_instance.initialize(config)
...
class BaseDBService:
    ...
    def retry_on_failure(self, func, max_retries: int = 3, delay: int = 1):
        """
        重試機制 - 處理臨時性網路錯誤
