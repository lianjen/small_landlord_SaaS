...
class TenantContactService(BaseDBService):
    ...
    def bind_line_user(
        ...
        Args:
            ...
            notify_rent: 是否接收租金通知（預設 True）
            notify_electricity: 是否接收電費通知（預設 True）
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    """
                    SELECT id, tenantname, roomnumber 
                    FROM tenants 
                ...
                if existing and existing[0] != tenant_id:
                    logger.warning(f"綁定失敗：LINE ID {line_user_id} 已綁定到房客 ID {existing[0]}")
                ...
                logger.info(f"✅ LINE 綁定成功: {tenant_name} ({room_number}) -> {line_user_id}")
        ...
        try:
            with self.get_connection() as conn:
                ...
                if not row:
                    # 如果沒有記錄，檢查房客是否存在
                    cursor.execute(
                        "SELECT tenantname, roomnumber FROM tenants WHERE id = %s",
                        (tenant_id,)
                    )
                    tenant = cursor.fetchone()
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE tc.line_user_id = %s AND t.isactive = true
                """, (line_user_id,))
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE t.isactive = true AND tc.line_user_id IS NOT NULL
    ...
    def update_notification_settings(
        ...
        Args:
            tenant_id: 房客 ID
            notify_rent: 是否接收租金通知（None 表示不更新）
            notify_electricity: 是否接收電費通知（None 表示不更新）
    ...
    def get_binding_statistics(self) -> Dict:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # 總房客數
                cursor.execute("SELECT COUNT(*) FROM tenants WHERE isactive = true")
...
if __name__ == "__main__":
    ...
    for binding in bindings[:5]:  # 只顯示前 5 筆
