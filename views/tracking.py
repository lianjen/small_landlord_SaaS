...
def render_rent_tracking(payment_service: PaymentService, tenant_service: TenantService):
    ...
    try:
        ...
        df["days_overdue"] = (today_ts - df["due_date_dt"]).dt.days
        ...
        df["overdue_display"] = df.apply(
            lambda row: f"🚨 逾期 {row['days_overdue']} 天"
        ...
        with col2:
            ...
            st.metric(
                "逾期",
                f"{total_overdue} 筆",
                delta="-" if total_overdue > 0 else "正常",
                delta_color="inverse",
            )

        with col3:
            total_amount = sum(
                safe_float(amount) for amount in df[df["status"] == "unpaid"]["amount"]
            )
            st.metric("待收金額", f"${total_amount:,.0f}")

        with col4:
            overdue_amount = sum(
                safe_float(amount) for amount in df[df["days_overdue"] > 0]["amount"]
            )
        ...
        df_sorted = df.sort_values(
            ["days_overdue", "due_date_dt"], ascending=[False, True]
        )

        df_sorted["amount_display"] = df_sorted["amount"].apply(
        ...
        st.dataframe(
            df_sorted[available_cols].rename(
                columns={
                    "room_number": "房號",
                    "tenant_name": "房客",
                    "payment_year": "年份",
...
def render_electricity_tracking(
    electricity_service: ElectricityService, tenant_service: TenantService
):
    """電費追蹤 - v3.0"""
    ...
    try:
        ...
        period_options = {
            f"{p['period_year']}/{p['period_month_start']:02d}-{p['period_month_end']:02d} (ID: {p['id']})": p[
                "id"
            ]
            for p in periods
        }
        ...
        with st.spinner("正在載入電費記錄..."):
            ...
            # records 可能已經是 DataFrame（新版 Service）
            ...
            if df.empty:
                st.warning(
                    f"📭 期間 ID {period_id} 尚無電費記錄，請前往「⚡ 電費管理」完成計算並儲存"
                )
                return

            # 統一欄位名稱（若 service 回傳英文欄位時）
            ...
            if not unpaid_df.empty:
                ...
                for idx, row in unpaid_df.iterrows():
                    ...
                    with col_info:
                        ...
                        st.write(
                            f"**{room}** | {tenant_type} | {total_usage} 度 | ${amount:,.0f} 元"
                        )
                    ...
                    with col_btn:
                        if st.button("✅", key=f"elec_pay_{period_id}_{idx}"):
                            with st.spinner(f"正在標記 {room}..."):
                                try:
                                    ok, msg = electricity_service.update_payment(
                                        period_id=period_id,
                                        room_number=room,
                                        new_status="paid",
...
def render_combined_tracking(
    ...
    try:
        ...
        rent_total = (
            sum(safe_float(p["amount"]) for p in rent_unpaid)
            if rent_unpaid
            else 0.0
        )
    ...
    try:
        ...
        if periods:
            ...
            st.info(
                f"📅 電費期間: {latest_period['period_year']}/"
                f"{latest_period['period_month_start']:02d}-"
                f"{latest_period['period_month_end']:02d}"
            )
            ...
            if isinstance(records_df, pd.DataFrame) and not records_df.empty:
                ...
                # 金額欄位
                if "應繳金額" in elec_unpaid_df.columns:
                    amount_series = elec_unpaid_df["應繳金額"]
                elif "amount" in elec_unpaid_df.columns:
                    amount_series = elec_unpaid_df["amount"]
                else:
    ...
    with col1:
        st.metric("🏠 租金待收", f"${rent_total:,.0f}", delta=f"{rent_count} 筆")

    with col2:
        st.metric("⚡ 電費待收", f"${elec_total:,.0f}", delta=f"{elec_count} 筆")
    ...
    with col4:
        ...
        st.metric("📊 收繳概況", f"{total_items} 筆待繳")
    ...
    with col_rent:
        ...
        if not rent_df.empty:
            ...
            st.dataframe(
                display_df[
                    ["room_number", "tenant_name", "payment_period", "amount_display"]
                ].rename(
                    columns={
                        "room_number": "房號",
                        "tenant_name": "房客",
    ...
    with col_elec:
        ...
        if elec_count > 0 and not elec_unpaid_df.empty:
            elec_display = elec_unpaid_df.copy()

            if "room_number" in elec_display.columns:
                elec_display = elec_display.rename(
                    columns={
                        "room_number": "房號",
                        "tenant_type": "類型",
                        "total_usage": "總度數",
            ...
            available_cols = [c for c in display_cols if c in elec_display.columns]
